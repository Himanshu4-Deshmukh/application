...
.timeline>li>.fa, .timeline>li>.glyphicon, .timeline>li>.ion {
  ...
  <div class="content-wrapper">
    <section class="content-header">
      <h3 style="font-size:20px !important">REAL-TIME STATUS RTA APPLIED FILES STATUS </h3>
      <ol class="breadcrumb">
            ...
            <table id="vehicle_tbl" class="table-bordered table-hover table table-striped" style="width:100%">
					        ...
					        <?php
								...
								// 			{
						        	...
						        	$rpp=2;
					        		//number of records
					        		// $sql = "SELECT * FROM track WHERE customer_id = '$customer'";
					        		$sql = "SELECT * FROM `track` LEFT JOIN `vehicle` ON track.vehicle_id = vehicle.id WHERE vehicle.customer_id='$customer'";
					        		$result = mysqli_query($conn, $sql);
					        		$num_rows = mysqli_num_rows($result);
					        		//number of pages
									...
									$cur_page = ($page-1)*$rpp;
									$c=($page*2)-1;
									$sql = "SELECT * FROM `track` LEFT JOIN `vehicle` ON track.vehicle_id = vehicle.id  WHERE vehicle.customer_id = '$customer' LIMIT " . $cur_page . ',' . $rpp;
									$result = mysqli_query($conn, $sql);
