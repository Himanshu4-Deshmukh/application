	...
	function userRegByAjax()
{
	...
	function getVehicleCountByUid($customer)
	{
		require('connection.php');
		$sel_query = "SELECT  vehicle.*   FROM `vehicle` WHERE `customer_id` = ".$customer;
		$result = mysqli_query($mysqli,$sel_query);
		$data = $result->num_rows;
		return $data;
	}

	function getExpVehicleDataByUid($customer,$field_name)
	{
		require('connection.php');
		$expdate = date("Y-m-d", strtotime("+15 days"));
		$nowdate = date("Y-m-d", strtotime("+0 days"));
		$sel_query = "SELECT user.firstname , user.lastname, user.id as uid ,vehicle.* FROM `vehicle` LEFT JOIN `user` ON vehicle.user_id = user.id WHERE vehicle.customer_id = ".$customer." AND " .$field_name . " <  '"  .$expdate ."' AND " .$field_name . " >  '"  .$nowdate ."' AND " .$field_name . " != '' ";
		$result = mysqli_query($mysqli,$sel_query);
...
function getExpCountVehicle($customer,$field_name)
	{
		require('connection.php');
		$expdate = date("Y-m-d", strtotime("+15 days"));
		$nowdate = date("Y-m-d", strtotime("+0 days"));
		
		$sel_query = "SELECT user.firstname , user.id as uid ,vehicle.* FROM `vehicle` LEFT JOIN `user` ON vehicle.user_id = user.id WHERE vehicle.customer_id = ".$customer." AND " .$field_name . " <  '"  .$expdate ."' AND " .$field_name . " >  '"  .$nowdate ."' AND " .$field_name . " != '' ";
// 		print_r($sel_query);die();
		$result = mysqli_query($mysqli,$sel_query);
    ...
    function getActiveCountVehicle($customer,$field_name1)
	{
		require('connection.php');
		$sel_query = "SELECT * FROM vehicle WHERE customer_id = ".$customer."  AND " .$field_name1 . " != '' AND " .$field_name1 . " != '1970-01-01'";
// 		$sel_query = "SELECT user.firstname , user.lastname, user.id as uid ,vehicle.* FROM `vehicle` LEFT JOIN `user` ON vehicle.user_id = user.id WHERE vehicle.customer_id = ".$customer."  AND " .$field_name1 . " != '' AND " .$field_name1 . " != '1970-01-01'";
// // 		print_r($sel_query);die();
		$result = mysqli_query($mysqli,$sel_query);
// 		print_R($result);die();
		if(!empty($result->num_rows))
...
// 		print_R($data);die();
	...
	function getActiveVehicleData($customer,$field_name1)
	{
		// $data = '';
		require('connection.php');
	...
	function getExpiredCountVehicle($customer,$field_name)
	{
		require('connection.php');
		$expdate = date("Y-m-d", strtotime("+15 days"));
		$nowdate = date("Y-m-d", strtotime("+0 days"));
		
		$sel_query = "SELECT user.firstname , user.id as uid ,vehicle.* FROM `vehicle` LEFT JOIN `user` ON vehicle.user_id = user.id WHERE vehicle.customer_id = ".$customer." AND " .$field_name . " <  '"  .$nowdate ."' AND " .$field_name . " != '1970-01-01 ' AND " .$field_name . " != ' '";
// 		print_r($sel_query);die();
		$result = mysqli_query($mysqli,$sel_query);
		if(!empty($result->num_rows))
		{
...
// 
	...
	function getExpirdVehicleData($customer,$field_name)
	{
		require('connection.php');
		$expdate = date("Y-m-d", strtotime("+15 days"));
		$nowdate = date("Y-m-d", strtotime("+0 days"));
		
		$sel_query = "SELECT user.firstname ,user.lastname , user.id as uid ,vehicle.* FROM `vehicle` LEFT JOIN `user` ON vehicle.user_id = user.id WHERE vehicle.customer_id = ".$customer." AND " .$field_name . " <  '"  .$nowdate ."' AND " .$field_name . " != '1970-01-01 ' AND " .$field_name . " != ' '";
		// print_r($sel_query);die();
		$result = mysqli_query($mysqli,$sel_query);
		if($result->num_rows)
		{
