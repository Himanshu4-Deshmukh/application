...
error_reporting(E_ALL);
   ...
   //  }
	...
	function getTotalV($id)
	{
		require('connection.php');
		$sel_query = "SELECT * FROM `display_status` WHERE `owner_id`=".$id;
	...
	{
		require('connection.php');
        //$sel_query = "SELECT track.* , user.id as uid , user.firstname as firstname,user.lastname as lastname,vehicle.id as vid,vehicle.user_id as vuid ,vehicle.va_customer as va_customer ,vehicle.reg_no  FROM `track` LEFT JOIN `vehicle` ON track.vehicle_id = vehicle.id  LEFT JOIN  `user` ON vehicle.user_id = user.id WHERE user.id = ".$id;
		$sel_query = "SELECT * FROM `track` LEFT JOIN `vehicle` ON track.vehicle_id = vehicle.id  LEFT JOIN  `user` ON vehicle.user_id = user.id WHERE user.id='$id'";
		$result = mysqli_query($mysqli,$sel_query);
			// print_R($sel_query);die();
		$data = null;
		if($result->num_rows)
	...
	{
		require('connection.php');
		$sel_query = "SELECT * FROM `track` RIGHT JOIN `vehicle` ON track.vehicle_id = vehicle.id  LEFT JOIN  `user` ON vehicle.user_id = user.id WHERE  track.complete = '0' AND user.id = ".$id;
		$result = mysqli_query($mysqli,$sel_query);
		$data = null;
		if($result->num_rows)
		{
	...
	function getCompleteDocument($id)
	{
		...
		require('connection.php');
		$sel_query = "SELECT *  FROM `track` RIGHT JOIN `vehicle` ON track.vehicle_id = vehicle.id  LEFT JOIN  `user` ON vehicle.user_id = user.id WHERE    track.complete = '1' AND user.id = ".$id;
		// print_R($sel_query);die();
		$result = mysqli_query($mysqli,$sel_query);
		$data = null;
		if($result->num_rows)
