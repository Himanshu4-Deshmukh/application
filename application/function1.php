...
error_reporting(E_ALL);
   ...
   //  }
	...
	{
		require('connection.php');
		$sel_query = "SELECT track.* , user.id as uid , user.firstname as firstname,user.lastname as lastname,vehicle.id as vid,vehicle.user_id as vuid ,vehicle.reg_no  FROM `track` 
		LEFT JOIN `vehicle` ON track.vehicle_id = vehicle.id  LEFT JOIN  `user` ON vehicle.user_id = user.id WHERE user.id = ".$id;
		// print_R($sel_query);die();
		$result = mysqli_query($mysqli,$sel_query);
		if($result->num_rows)
		{
	...
	function getPendingDocument($id)
	{
		require('connection.php');
		$sel_query = "SELECT track.* , user.id as uid , user.firstname as firstname,user.lastname as lastname,vehicle.id as vid,vehicle.user_id as vuid ,vehicle.reg_no FROM `track` 
		RIGHT JOIN `vehicle` ON track.vehicle_id = vehicle.id  LEFT JOIN  `user` ON vehicle.user_id = user.id WHERE    track.complete = '0' AND user.id = ".$id;
		// print_R($sel_query);die();
		$result = mysqli_query($mysqli,$sel_query);
		if($result->num_rows)
		{
