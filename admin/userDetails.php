			...
			<?php
                    ...
                    $aa ="'". implode("', '",explode(', ',$assigned_users)) ."'";
                 	if($id == 1)
			        	{
								          ...
								          <?php
					        				echo'<td style="text-align:center"><a href="'.$img.'" target="_blank"><img src = '.$img.' height="50" width="50"></a></td>
					        					...
					        					<a href ="viewUserVDetails.php?id='.$val["id"].'" target="_blank" class="btn btn-primary">
					        						<i class="fa fa-car"></i><br>'.getVehicleCountByUid($val['id'],$val['user_type']).'</a></td>
				        ...
				        }   //end for admin ($id=1) and choose assign users
                        else{
                                    if(isset($_POST["search"]))
                                    {
                                    echo '<script>
                                    ...
                                    $s_item = strtolower($_POST["search"]);
                                    $rpp=10;
                                    $sql="SELECT * FROM user WHERE email IN ($aa)";
                                    $result=mysqli_query($conn,$sql);
                                    $num_rows = mysqli_num_rows($result);
                                    $num_pages = ceil($num_rows/$rpp);
                                    if (!isset($_GET['page'])) {
                                        ...
                                        ?>
                                                <td style="text-align:center">
                                                    ...
                                                    $row=mysqli_fetch_assoc($result);
                                                     ...
                                                     echo $company_name;
                                                    ?></td>
                                    <?php echo'<td style="text-align:center"><a href="'.$img.'" target="_blank"><img src = '.$img.' height="50" width="50"></a></td>
                                                <td style="text-align:center">'.$val["firstname"]." ".$val["lastname"].'</td>
                                                <td style="text-align:center">'.$val["email"].'</td>
                                                <td style="text-align:center">'.$val["mobile_no"].'</td>
                                                <td style="text-align:center">'.$val["gender"].'</td>
                                    ...
                                    $cur_page = ($page-1)*$rpp;
                                    $c=($page*10)-9;
                                    $sql="SELECT * FROM user WHERE  email IN ($aa) LIMIT " . $cur_page . ',' . $rpp;
                                    $result=mysqli_query($conn,$sql);
                                    while($row=mysqli_fetch_assoc($result))
                                    {
                                        $data[] = $row;
